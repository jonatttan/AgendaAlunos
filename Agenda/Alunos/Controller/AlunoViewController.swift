...
class AlunoViewController: UIViewController, ImagePickerFotoSelecionada {
    ...
    @IBAction func buttonSalvar(_ sender: UIButton) {
        let json = montaDicionarioDeParametros() // Instancia o dicionario JSON
        Repositorio().salvaAluno(aluno: json) // Chama a função dentro da classe Repositorio passando o objeto dicionario de dados
